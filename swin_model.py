...
def train(train_images_path, train_images_label, val_images_path=None, val_images_label=None, classes=2, epochs=20, batch_size=32,
          lr=0.0001, weight='./init/swin_tiny_patch4_window7_224.pth', freeze=False, device='cuda:0', val=True, data=1, init=True):
    ...
    for epoch in range(epochs):
        ...
        f = open('./res_dir/train_res/data'+str(data)+'/swin_res.txt', 'a')
        ...
        if best_acc < train_acc:
            best_acc = train_acc
            best_epoch = epoch
            torch.save(model.state_dict(), './res_dir/weights/data'+str(data)+'/best_swin_model.pth')
...
def predict(images_path, images_label, num_class=2, batch_size=8, data=1):
    ...
    model_weight_path = './res_dir/weights/data'+str(data)+'/best_swin_model.pth'
    ...
    pd.DataFrame(np.array(score)).to_csv('./res_dir/label/data' + str(data) + '/swin' + '.csv', header=None, index=None)
    return score
